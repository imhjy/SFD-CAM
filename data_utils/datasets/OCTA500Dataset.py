...
class OCTA500Dataset(Dataset):
    ...
    def process_data(self, label_dir_name='GT_Label'):
        dataset_list = {}
        modality = ['OCT', 'OCTA']
        data_dir = os.path.join(self.root, self.type)
        strat_id = -1
        end_id = -1
        for modal in modality:  # OCT/OCTA
            dataset_list.update({modal: {}})
            ct_list = os.listdir(os.path.join(data_dir, modal))
            ct_list = natsort.natsorted(ct_list)
            strat_id = ct_list.index(ct_list[0])
            end_id = ct_list.index(ct_list[-1]) + 1
            for ct in ct_list[strat_id:end_id]:
                dataset_list[modal].update({ct: {}})
                scan_list = os.listdir(os.path.join(data_dir, modal, ct))
                scan_list = natsort.natsorted(scan_list)
                for i in range(0, len(scan_list)):
                    scan_list[i] = os.path.join(data_dir, modal, ct, scan_list[i])
                dataset_list[modal][ct] = scan_list

        dataset_list.update({'Label': {}})
        ct_list = os.listdir(os.path.join(data_dir, label_dir_name))
        ct_list = natsort.natsorted(ct_list)
        for ct in ct_list[strat_id:end_id]:
            dataset_list['Label'].update({ct.split('.')[0]: {}})
            label_address = os.path.join(data_dir, label_dir_name, ct)
            dataset_list['Label'][ct.split('.')[0]] = label_address

        dataset_binary_list = {modal: [] for modal in modality}
        for modal in modality:
            dataset_binary_list[modal] = np.zeros((self.hypes['dataset']['block_size'][0],
                                                   self.hypes['dataset']['data_size'][1],
                                                   self.hypes['dataset']['data_size'][2],
                                                   end_id - strat_id), dtype=np.uint8)
            ct_list = list(dataset_list[modal])
            ct_num = -1
            for ct in ct_list:
                ct_num += 1
                scan_list = list(dataset_list[modal][ct])
                scan_num = -1
                for scan in scan_list:
                    scan_num += 1
                    # dataset_binary_list[modal][:, :, scan_num, ct_num] = np.array(imageio.imread(scan))
                    dataset_binary_list[modal][:, :, scan_num, ct_num] = np.array(
                        Image.fromarray(imageio.imread(scan)).resize(
                            (self.hypes['dataset']['data_size'][1],
                             self.hypes['dataset']['block_size'][0])))  # 把640的高度压缩到160
        dataset_binary_list['Label'] = np.zeros((1,
                                                 self.hypes['dataset']['data_size'][1],
                                                 self.hypes['dataset']['data_size'][2],
                                                 end_id - strat_id), dtype=np.uint8)

        ct_list = list(dataset_list['Label'])
        ct_num = -1
        for ct in ct_list:
            ct_num += 1
            label_address = dataset_list['Label'][ct]
            dataset_binary_list['Label'][0, :, :, ct_num] = np.array(imageio.imread(label_address))

        save_root = os.path.join(self.root, self.type, 'processed')  # 处理后的数据保存目录
        f = h5pickle.File(os.path.join(os.path.join(save_root, "data.hdf5")), "w")

        # 存的话还是存一起，不然取数据太慢了
        # f.create_dataset('OCT', data=dataset_binary_list['OCT'])
        # f.create_dataset('OCTA', data=dataset_binary_list['OCTA'])
        f.create_dataset('Images',
                         data=np.stack([dataset_binary_list['OCT'], dataset_binary_list['OCTA']],
                                       axis=0))  # (2, C, H, W))
        f.create_dataset('Label', data=dataset_binary_list['Label'])
        f.close()
