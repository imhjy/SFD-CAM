...
class RimOneDataset(Dataset):
    ...
    def __init__(self, hypes, train: bool, transforms=None, fold=0):
        super(RimOneDataset, self).__init__()
        ...
        if fold == 0:
            ...
            self.label = [
                os.path.join(data_root, "label", f"{i.split('.')[0]}-1-Cup-T.png")
                for i in img_names]
            # check files
            for i in self.label:
    ...
    def __getitem__(self, idx):
        ...
        cup_processed = np.where(cup_arr >= 255, 0, 255).astype(np.uint8)
        disc_processed = np.where(disc_arr >= 255, 128, 255).astype(np.uint8)

        # 3. 验证cup区域是否完全在disc区域内
        # 若cup标记为2的位置对应disc标记不是1，则报错
        conflict_mask = (cup_processed == 0) & (disc_processed != 128)
        if np.any(conflict_mask):
            print('cup区域不完全在disc区域内, 已经重新设置')
            # 条件：cup_processed为0且disc_processed不为128
            condition = (cup_processed == 0) & (disc_processed != 128)

            # 修改满足条件的像素为255，其他保持不变
            cup_processed = np.where(condition, 255, cup_processed)
        ...
        # 2. 找到ROI区域(RIM-ONE跳过, 但是要设置大小都为480)
        ...
        img = img.resize((self.roi_size,self.roi_size), Image.Resampling.LANCZOS)
        label = label.resize((self.roi_size,self.roi_size), Image.Resampling.NEAREST)
    ...
    def fold_dataset_split(hypes):
        ...
        def get_label_path_list(images_list):
            ...
            for path in images_list:
                ...
                cup_list.append(os.path.join('label', f'{file_name}-1-Cup-T.png'))
                disc_list.append(os.path.join('label', f'{file_name}-1-Disc-T.png'))
