...
class GammaDataset(Dataset):
    ...
    def __init__(self, hypes, train: bool, transforms=None, fold=0):
        super(GammaDataset, self).__init__()
        ...
        if fold == 0:
            data_root = root
            img_names = [i for i in os.listdir(os.path.join(data_root, "training\\images")) if i.endswith(".jpg")]
            self.img_list = [os.path.join(data_root, "training\\images", i) for i in img_names]
            self.label = [os.path.join(data_root, "training\\mask", i.split('.')[0] + '.png')
                          for i in img_names]
            # check files
            for i in self.label:
