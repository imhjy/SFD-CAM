...
class RefugeDataset(Dataset):
    ...
    def __init__(self, hypes, train: bool, transforms=None, fold=0):
        ...
        if fold == 0:
            data_root = root
            img_names = [i for i in os.listdir(os.path.join(data_root, "train/images")) if i.endswith(".png")]
    ...
    def fold_dataset_split(hypes):
        ...
        def get_image_number(dataset_path):
            ...
            return len(
                ...
                os.listdir(os.path.join(val_path, 'images'))
            )
        ...
        def get_image_path_list(dataset_path):
            ...
            images_list = np.array(
                [os.path.join('train', 'images', file) for file in os.listdir(os.path.join(train_path, 'images'))] +
                [os.path.join('test', 'images', file) for file in os.listdir(os.path.join(test_path, 'images'))] +
                [os.path.join('val', 'images', file) for file in os.listdir(os.path.join(val_path, 'images'))]

            )
            return images_list

        def get_label_path_list(dataset_path):
            train_path = os.path.join(dataset_path, 'train')
            test_path = os.path.join(dataset_path, 'test')
            val_path = os.path.join(dataset_path, 'val')
            label_list = np.array(
                [os.path.join('train', 'mask', file) for file in os.listdir(os.path.join(train_path, 'mask'))] +
                [os.path.join('test', 'mask', file) for file in os.listdir(os.path.join(test_path, 'mask'))] +
                [os.path.join('val', 'mask', file) for file in os.listdir(os.path.join(val_path, 'mask'))]
        ...
        label_list = get_label_path_list(dataset_path)
