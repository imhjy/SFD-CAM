...
import numpy as np
from models.sub_model.EyeNet.modules import GTFD, EFEA, LDCA
...
class MedNeXtInstance(nn.Module):
    ...
    def __init__(self,
        ...
        self.GTFD = GTFD(layer_dims=[n_channels, n_channels * 2, n_channels * 4, n_channels * 8])
        self.atts = nn.Sequential(
            EFEA(in_channels=n_channels),
            EFEA(in_channels=n_channels * 2),
            EFEA(in_channels=n_channels * 4),
            EFEA(in_channels=n_channels * 8)
        )
        self.LDCA = LDCA(in_channel_global=n_channels * 8, in_channel_high=n_channels * 16)
    ...
    def forward(self, x):  # [batch, channel, H, W, L] or # [batch, channel, H, W] [1,2,128,128,128]
        ...
        if self.outside_block_checkpointing:
            ...
            # dec_x = x_res_3 + x_up_3
            # dec_x = GTFD_output_list[3] + x_up_3
            dec_x = self.atts[3](GTFD_output_list[3], x_res_3) + x_up_3
            x = self.dec_block_3(dec_x)  # [1,256,16,16]

            if self.do_ds:
                x_ds_3 = self.out_3(x)
            del x_res_3, x_up_3

            x_up_2 = self.up_2(x)  # [1,128,32,32]
            # dec_x = x_res_2 + x_up_2
            # dec_x = GTFD_output_list[2] + x_up_2
            dec_x = self.atts[2](GTFD_output_list[2], x_res_2) + x_up_2
            x = self.dec_block_2(dec_x)  # [1,128,32,32]
            if self.do_ds:
                x_ds_2 = self.out_2(x)
            del x_res_2, x_up_2

            x_up_1 = self.up_1(x)  # [1,64,64,64]
            # dec_x = x_res_1 + x_up_1
            # dec_x = GTFD_output_list[1] + x_up_1
            dec_x = self.atts[1](GTFD_output_list[1], x_res_1) + x_up_1
            x = self.dec_block_1(dec_x)  # [1,64,64,64]
            if self.do_ds:
                x_ds_1 = self.out_1(x)
            del x_res_1, x_up_1

            x_up_0 = self.up_0(x)  # [1,32,128,128]
            # dec_x = x_res_0 + x_up_0
            # dec_x = GTFD_output_list[0] + x_up_0
            dec_x = self.atts[0](GTFD_output_list[0], x_res_0) + x_up_0
            x = self.dec_block_0(dec_x)  # [1,32,128,128]
            del x_res_0, x_up_0, dec_x
...
def EyeNet123(in_channels=1, num_classes=2, model_id='S', kernel_size=3, deep_supervision=False):
    model_dict = {
...
if __name__ == '__main__':
    ...
    print('GMAC: ', macs, 'params: ', params)  # GMAC:  172.1 GMac params:  42.77 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit)) # GFlops: 344.2 GFlops
