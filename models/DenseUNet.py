...
class DenseUNet(nn.Module):
    """
    2017
    Guan S, Khan A A, Sikdar S, et al. Fully dense UNet for 2-D sparse photoacoustic tomography artifact removal[J].
    IEEE journal of biomedical and health informatics, 2019, 24(2): 568-576.
    """
...
if __name__ == '__main__':
    ...
    model = DenseUNet(img_ch=1, output_ch=2).to(device)
    input_1 = torch.rand(2, 1, 224, 224).to(device)
    print(input_1.shape)
    macs, params = get_model_complexity_info(model, (1, 480, 480), as_strings=True, print_per_layer_stat=True)
    print('GMAC: ', macs, 'params: ', params)  # flops:  278.98 GMac params:  18.36 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 557.96 GFlops
