...
def u2net_full(in_ch=1, out_ch: int = 2):
    cfg = {
    ...
    return U2NetInstance(cfg, out_ch)


def u2net_lite(in_ch=1, out_ch: int = 2):
    cfg = {
        # height, in_ch, mid_ch, out_ch, RSU4F, side
...
def U2Net(in_ch=1, out_ch=2,mode='full'):
    if mode == 'full':
        return u2net_full(in_ch,out_ch)
    elif mode == 'lite':
        return u2net_lite(in_ch,out_ch)
...
if __name__ == '__main__':
    ...
    model = u2net_full(in_ch=1, out_ch=2)
    ...
    print('GMAC: ', macs, 'params: ', params)  # flops:  132.36 GMac params:  44.01 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 264.72 GFlops
