...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self, config, patchsize, img_size, in_channels):
        ...
        n_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])  # 224 / 16 = 14 n_patch = 196
...
class Reconstruct(nn.Module):
    ...
    def forward(self, x):
        ...
        x = nn.Upsample(scale_factor=self.scale_factor[0])(x)
...
class UCTransNetInstance(nn.Module):
    """
        [AAAI2022] UCTransNet: Rethinking the Skip Connections in U-Net from a Channel-Wise Perspective with Transformer
        https://github.com/McGregorWwww/UCTransNet
        论文链接 https://ojs.aaai.org/index.php/AAAI/article/view/20144
    """

    def __init__(self, config, n_channels=3, n_classes=1, img_size=224, vis=False, **kwargs):
        super().__init__()
        self.vis = vis
...
def UCTransNet(n_channels=1, n_classes=2, img_size=224):
    config_vit = get_CTranS_config()
    model = UCTransNetInstance(config_vit, n_channels=n_channels, n_classes=n_classes, img_size=img_size)
    return model
...
if __name__ == '__main__':
    ...
    config_vit = get_CTranS_config()
    model = UCTransNetInstance(config_vit, n_channels=1, n_classes=5, img_size=128)

    t1 = torch.randn((2, 1, 128, 128))
    print(model(t1)['out'].shape)
    macs, params = get_model_complexity_info(model, (1, 128, 128))
    print('GMAC: ', macs, 'params: ', params)  # GMAC:  113.77 GMac params:  66.79 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 227.54 GFlops
