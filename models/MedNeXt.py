...
class MedNeXtInstance(nn.Module):
    ...
    def forward(self, x):  # [batch, channel, H, W, L] or # [batch, channel, H, W] [1,2,128,128,128]
        ...
        if self.outside_block_checkpointing:
            ...
            x_res_0 = self.enc_block_0(x)  # [1,32,128,128,128]
            x = self.down_0(x_res_0)  # [1,64,64,64,64]
            x_res_1 = self.enc_block_1(x)  # [1,64,64,64,64]
            x = self.down_1(x_res_1)  # [1,128,32,32,32]
            x_res_2 = self.enc_block_2(x)  # [1,128,32,32,32]
            x = self.down_2(x_res_2)  # [1,256,16,16,16]
            x_res_3 = self.enc_block_3(x)  # [1,256,16,16,16]
            x = self.down_3(x_res_3)  # [1,512,8,8,8]

            x = self.bottleneck(x)  # [1,512,8,8,8]
            if self.do_ds:  # 是否做深度监督
                x_ds_4 = self.out_4(x)

            x_up_3 = self.up_3(x)  # [1,256,16,16,16]
            dec_x = x_res_3 + x_up_3
...
def MedNeXt(in_channels=1, num_classes=2, model_id='S', kernel_size=3, deep_supervision=False):
    model_dict = {
...
if __name__ == '__main__':
    ...
    import re
    model = create_mednextv1_base(1, 2, 3, True)
    ...
    print('GMAC: ', macs, 'params: ', params) # GMAC:  53.26 GMac params:  10.45 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit)) # GFlops: 106.52 GFlops
