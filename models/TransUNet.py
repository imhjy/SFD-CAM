...
def compute_mhsa(q, k, v, scale_factor=1, mask=None):
    ...
    if mask is not None:
        ...
        scaled_dot_prod = scaled_dot_prod.masked_fill(mask, -1e-10)
...
class TransUNet(nn.Module):
    def __init__(self, *, img_dim, in_channels, classes,
                 ...
                 vit_channels=None,**kwargs
                 ):
        """
        My reimplementation of TransUnet based on the paper: 2021年
...
if __name__ == '__main__':
    ...
    import re
    model = TransUNet(img_dim=128, in_channels=1, classes=5,patch_size=8)
    t1 = torch.randn((2, 1, 128, 128)) # 必须是 128 的倍数
    print(model(t1)['out'].shape)
    macs, params = get_model_complexity_info(model, (1, 128, 128), as_strings=True, print_per_layer_stat=True)
    print('GMAC: ', macs, 'params: ', params)  # flops:  88.22 GMac params:  209.53 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 176.44 GFlops
    pass