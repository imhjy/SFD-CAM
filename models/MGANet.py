...
class GPG_1(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        feat = torch.cat(
            [self.ECA(self.dilation1(feat)), self.ECA(self.dilation2(feat)), self.ECA(self.dilation3(feat)),
             self.ECA(self.dilation4(feat)), self.ECA(self.dilation5(feat))], dim=1)
...
class GPG_2(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        feat = torch.cat(
            [self.ECA(self.dilation1(feat)), self.ECA(self.dilation2(feat)), self.ECA(self.dilation3(feat)),
             self.ECA(self.dilation4(feat))], dim=1)
        feat = self.conv_out(feat)
        return feat
...
class GPG_3(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        feat = torch.cat(feats, dim=1)
        feat = torch.cat(
            [self.ECA(self.dilation1(feat)), self.ECA(self.dilation2(feat)), self.ECA(self.dilation3(feat))], dim=1)
        feat = self.conv_out(feat)
        return feat
...
class MGANet(nn.Module):
    """
    https://www.sciencedirect.com/science/article/pii/S026322412301117X
...
if __name__ == '__main__':
    ...
    model = MGANet(img_ch=1, output_ch=2)
    ...
    print('GMAC: ', macs, 'params: ', params)  # flops:  394.42 GMac params:  40.69 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 788.84 GFlops
