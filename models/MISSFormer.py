...
class MISSFormer(nn.Module):
    """
    UCTransNet: An Effective Medical Image Segmentation Transformer
    """

    def __init__(self, num_classes=9, image_size=224, token_mlp_mode="mix_skip", encoder_pretrained=True):
        super().__init__()

        reduction_ratios = [8, 4, 2, 1]
        heads = [1, 2, 5, 8]
        d_base_feat_size = image_size // 32  # 16 for 512 inputsize   7for 224
...
if __name__ == '__main__':
    ...
    model = MISSFormer(num_classes=5,image_size=128).cuda()
    t1 = torch.randn((4, 1, 128, 128)).cuda()
    print(model)
    print(model(t1)['out'].shape)
    # macs, params = get_model_complexity_info(model, (1, 224, 224), as_strings=True, print_per_layer_stat=True)
    # print('GMAC: ', macs, 'params: ', params)  # GMAC:  7.26 GMac params:  42.46 M   GMAC =（乘法累加运算次数）/（10⁹）
    # # Extract the numerical value
    # flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # # Extract the unit
    # flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    # print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 14.52 GFlops
