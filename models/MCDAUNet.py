...
class M_Decoder(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, kernel_size=3, dilation=1, deconv=False, bn=False,
        ...
        self.conv1 = ConvBnRelu2d(input_channels, output_channels, kernel_size=kernel_size, padding=padding,
                                  dilation=dilation, stride=1, groups=1, is_bn=bn, BatchNorm=BatchNorm,
                                  num_groups=num_groups)
        self.conv2 = ConvBnRelu2d(output_channels, output_channels, kernel_size=kernel_size, padding=padding,
                                  dilation=dilation, stride=1, groups=1, is_bn=bn, BatchNorm=BatchNorm,
                                  num_groups=num_groups)
        self.conv3 = ConvBnRelu2d(output_channels, output_channels, kernel_size=kernel_size, padding=padding,
                                  dilation=dilation, stride=1, groups=1, is_bn=bn, BatchNorm=BatchNorm,
                                  num_groups=num_groups)
...
class CCAF2(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.ch_conv = ConvBnRelu2d(in_channels * 2, out_channels, kernel_size=3, is_bn=True, is_relu=True,
                                    padding=(3 - 1) // 2)

        self.conv1 = ConvBnRelu2d(in_channels, in_channels, kernel_size=3, dilation=2, padding=2, is_bn=True,
                                  is_relu=True)
...
class CCAF3(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.softamx = nn.Softmax2d()
        self.ch_conv = ConvBnRelu2d(in_channels * 3, out_channels, kernel_size=3, is_bn=True, is_relu=True,
                                    padding=(3 - 1) // 2)
        ...
        self.conv2 = ConvBnRelu2d(in_channels, in_channels, kernel_size=3, dilation=4, padding=4, is_bn=True,
                                  is_relu=True)
        self.conv3 = ConvBnRelu2d(in_channels, in_channels, kernel_size=3, dilation=8, padding=8, is_bn=True,
                                  is_relu=True)
...
class CCAF4(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.ch_conv = ConvBnRelu2d(in_channels * 4, out_channels, kernel_size=3, is_bn=True, is_relu=True,
                                    padding=(3 - 1) // 2)
        ...
        self.conv1 = ConvBnRelu2d(in_channels, in_channels, kernel_size=3, dilation=1, padding=1, is_bn=True,
                                  is_relu=True)
        self.conv2 = ConvBnRelu2d(in_channels, in_channels, kernel_size=3, dilation=2, padding=2, is_bn=True,
                                  is_relu=True)
        self.conv3 = ConvBnRelu2d(in_channels, in_channels, kernel_size=3, dilation=4, padding=4, is_bn=True,
                                  is_relu=True)
        self.conv4 = ConvBnRelu2d(in_channels, in_channels, kernel_size=3, dilation=8, padding=8, is_bn=True,
                                  is_relu=True)
...
class MCDAUNet(nn.Module):
    """
    https://www.sciencedirect.com/science/article/pii/S0010482523007345
    2023
    """

    def __init__(self, in_channels=1, n_classes=2, bn=True, BatchNorm=False):
        super(MCDAUNet, self).__init__()

        # the down convolution contain concat operation   两次卷积 +
        self.down1 = M_Encoder(in_channels, 32, kernel_size=3, bn=bn, BatchNorm=BatchNorm, res=True)  # 512
...
if __name__ == '__main__':
    ...
    import re
    model = MCDAUNet(in_channels=1, n_classes=2)
    t1 = torch.randn((2, 1, 224, 224))
    print(model(t1)['out'].shape)
    macs, params = get_model_complexity_info(model, (1, 224, 224), as_strings=True, print_per_layer_stat=True)
    print('GMAC: ', macs, 'params: ', params)  # flops:  146.11 GMac params:  13.8 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 292.22 GFlops
