...
class SwinUNet(nn.Module):
    r""" Swin-unet: UNet-like pure transformer for medical image segmentation
...
if __name__ == '__main__':
    ...
    model = SwinUNet(in_chans=1, num_classes=2, img_size=480, depths=[2, 3, 5, 8],window_size=5)
    t1 = torch.randn((2, 1, 480, 480))  # 必须是 32 * window_size(7) == 224 的倍数
    print(model)
    # print(model(t1).shape)
    macs, params = get_model_complexity_info(model, (1, 480, 480), as_strings=True, print_per_layer_stat=True)
    print('GMAC: ', macs, 'params: ', params)  # flops:  23.62 GMac params:  27.17 M   GMAC =（乘法累加运算次数）/（10⁹）
    # Extract the numerical value
    flops = eval(re.findall(r'([\d.]+)', macs)[0]) * 2
    # Extract the unit
    flops_unit = re.findall(r'([A-Za-z]+)', macs)[0][0]
    print('GFlops: {} {}Flops'.format(flops, flops_unit))  # GFlops: 47.24 GFlops
