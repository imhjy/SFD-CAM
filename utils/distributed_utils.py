...
class DiceCoefficient(object):
    """
    求Dice系数
    https://www.bilibili.com/video/BV1rq4y1w7xM
    '''
...
class MetricLogger(object):
    def __init__(self, delimiter="\t"):
        """
            meters: 是一个字典, 托管了想要的字段
            delimiter: 用于分割不同的指标值
        """
        self.meters = defaultdict(SmoothedValue)  # defaultdict 当访问不存在的键时会自动创建默认值
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0:
                ...
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))  # 剩余时间
                if torch.cuda.is_available():
                    # (第几个loader,剩余时间,其他需要看的参数,本次loader使用的时间/秒,处理数据的时间/秒,还能分配的显存/MB )
